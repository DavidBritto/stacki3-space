...
class ZshrcPreservationTest(unittest.TestCase):
    def test_installer_preserves_existing_zshrc_by_default(self):
        ...
        self.assertIn("STACKI3_OVERWRITE_ZSHRC", text)
    ...
    def test_docs_explain_zshrc_overwrite_escape_hatch(self):
        for path in [README, NEW_MACHINE]:
            text = path.read_text()
            self.assertIn("STACKI3_OVERWRITE_ZSHRC=1 bash install.sh", text)

    def test_remote_bootstrap_can_install_dependencies(self):
        ...
        for path in [README, NEW_MACHINE]:
            text = path.read_text()
            self.assertIn("bash -s -- --deps", text)
